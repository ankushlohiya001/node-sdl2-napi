...
class render {
	...
	get gfx() {
		this._gfx = this._gfx || new (require('sdl2-gfx-napi').class('primitives'))(this._render)
		return this._gfx
	}
	...
	readPixels(rect, pitch) {
		...
		if(rect) {
			...
			_rect = this.outputSize
		}
		pitch = pitch || 4

		let buf = new Buffer(_rect.w * _rect.h * pitch)
	...
	set target(tt) {
		SDL_render.SDL_SetRenderTarget(this._render, tt._texture)
	}
